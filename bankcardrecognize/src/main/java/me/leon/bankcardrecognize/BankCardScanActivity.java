...
public class BankCardScanActivity extends AppCompatActivity implements Callback, PreviewCallback {
    ...
    public static final String RESULT = "StringR";
    public static final String IMAGE_ARR = "PicR";
        ...
        this.flash.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (!BankCardScanActivity.this.getPackageManager().hasSystemFeature("android.hardware.camera.flash")) {
                    Toast.makeText(BankCardScanActivity.this, BankCardScanActivity.this.getResources().getString(BankCardScanActivity.this.getResources().getIdentifier("toast_flash", "string", BankCardScanActivity.this.getApplication().getPackageName())), Toast.LENGTH_SHORT).show();
                } else if (BankCardScanActivity.this.camera != null) {
                    Parameters parameters = BankCardScanActivity.this.camera.getParameters();
                    ...
                    try {
                        BankCardScanActivity.this.camera.setParameters(parameters);
                    } catch (Exception e) {
                        Toast.makeText(BankCardScanActivity.this, BankCardScanActivity.this.getResources().getString(BankCardScanActivity.this.getResources().getIdentifier("toast_flash", "string", BankCardScanActivity.this.getApplication().getPackageName())), Toast.LENGTH_SHORT).show();
                    }
                    BankCardScanActivity.this.camera.startPreview();
                }
            }
        });
    }
        ...
        try {
            ...
            if (this.timer == null) {
                this.timer = new TimerTask() {
                    @Override
                    public void run() {
                        if (BankCardScanActivity.this.camera != null) {
                            try {
                                BankCardScanActivity.this.camera.autoFocus(new AutoFocusCallback() {
                                    @Override
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        if (this.counter == 2) {
            ...
            if (isBorders[0] != 1 || isBorders[1] != 1 || isBorders[2] != 1 || isBorders[3] != 1) {
               ...
               // intent.putExtra(IMAGE_ARR, pLineWarp);
                intent.putExtra(RESULT, String.valueOf(recogval).trim());
